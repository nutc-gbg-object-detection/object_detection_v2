...
def visualize_boxes_and_labels_on_image_array(
  ...
  list_of_coordinates = [filename]
  # Draw all boxes onto image.
  for box, color in box_to_color_map.items():
    ymin, xmin, ymax, xmax = box

    list_of_coordinates.append((int(xmin*265), int(ymin*100), int(xmax*265), int(ymax*100)))
    #print([int(xmin*265), int(ymin*100), int(xmax*265), int(ymax*100)])                         # ------------------------------------------------------------
    if instance_masks is not None:
  ...
  with open('file1.txt', "wb") as fp:
    pickle.dump(list_of_coordinates, fp)
    fp.close
    ##Okay
  return image
